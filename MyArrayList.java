...
public class MyArrayList<T> implements MyList<T> {
    private int size = 0;
    private static final int d_s = 10;
    private T[] array;


    // Creating MyArraylist by using d_s
    public MyArrayList() {
        array = (T[]) new Object[d_s];
    }

    // creating MyarrayList
    public MyArrayList(int a_s) {
        if (a_s< 0)
            throw new IllegalArgumentException("Illegal size: " + a_s);
        array = (T[]) new Object[a_s];
        size = 0;
    }
